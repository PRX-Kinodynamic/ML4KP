...
{
    auto params = param_loader("examples/basic/sst.yaml", argc, argv);
    ...
    // sst_spec.valid_state = [](space_point_t& s)
    // {
        // Custom valid_state can be added here.  
    // };

    // sst_spec.valid_check = [&sst_spec](trajectory_t& traj)
    // {
    ...
    // sst_spec.sample_plan = [&](plan_t& plan, space_point_t pose)
    // {
    ...
    vis_group -> add_vis_infos(info_geometry_t::LINE, sst_query.tree_visualization, 
        body_name, ss);

    vis_group -> add_detailed_vis_infos(info_geometry_t::FULL_LINE, sst_query.solution_traj, 
        body_name, ss);
